...
const AddFertilizer = () => {
  ...
  const handleAddProduct = async (dealerId) => {
    try {
      if (quantity <= 0 || price <= 0 || itemcode < 0) {
        setErrorMessage(
          'Item code, quantity, and price must be positive values.'
        );
        return;
      }
      ...
      const response = await axios.post(
        'http://localhost:8070/dealer/addProduct',
        {
          id: dealerId,
          name: product,
          price: price,
  ...
  return (
    <div
      className="section add-items-section green-bg"
      style={{ position: 'relative', padding: '20px' }}
    >
      <Typography variant="h4">
        <AddCircleOutline
          style={{ fontSize: '32px', marginRight: '8px', color: 'black' }}
        />
        Add Items
      </Typography>
      <FormControl
      ...
      <div style={{ textAlign: 'center', marginTop: '30px' }}>
        <Button
          variant="contained"
          onClick={() => handleAddProduct(dealerId)}
          sx={{
            borderRadius: '20px',
            boxShadow: '0 5px 6px rgba(0, 0, 0, 0.6)',
            fontSize: '15px',
