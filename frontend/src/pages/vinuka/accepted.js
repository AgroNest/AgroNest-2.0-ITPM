    ...
    const fetchAcceptedRequests = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8070/testRequest/retrieveAcceptedTestRequests/${labId}`
        );
  ...
  const handleStatusChange = async (event, requestId) => {
    ...
    try {
      ...
      if (newStatus === 'completed') {
        await axios.put('http://localhost:8070/labAccount/incrementCompleted', {
      ...
      setAcceptedRequests(
        acceptedRequests.filter((request) => request._id !== requestId)
      );
    } catch (error) {
