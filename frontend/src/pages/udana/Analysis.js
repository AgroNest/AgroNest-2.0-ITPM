...
const ShopAnalysis = () => {
  ...
  const fetchFertilizerDataForChart = async () => {
    try {
      ...
      const fertilizersResponse = await axios.get(
        `http://localhost:8070/dealer/${dealerId}/fertilizers`
      );
  ...
  const handleSearch = (dealerId) => {
    ...
    axios
      .get(
        `http://localhost:8070/dealer/fertilizers/search?dealerId=${dealerId}&term=${searchTerm}`
      )
      .then((response) => {
  ...
  const handleSearchChange = (event, dealerId) => {
    ...
    axios
      .get(
        `http://localhost:8070/dealer/fertilizers/search?dealerId=${dealerId}&term=${value}`
      )
      .then((response) => {
      ...
      <div className="content">
        <div className="section view-items-section light-green-bg">
          <Typography variant="h4">
            <SearchIcon
              style={{ fontSize: 32, color: 'black', marginRight: 8 }}
            />
            Search Fertilizers
          </Typography>
          <div
              ...
              {suggestions.map((fertilizer, index) => (
                ...
                >
                  <ListItemText
                    ...
                    secondary={`Item Code: ${fertilizer.itemcode}`}
                  />
                </ListItem>
              ))}
            </List>
          ...
          <PDFExport
            ref={(component) => setPdfExportComponent1(component)}
            paperSize="B4"
          >
            {selectedFertilizer && (
              <div
              ...
              >
                <div className="img-container">
                  <img
                    src={getFertilizerImage(selectedFertilizer[0].name)}
                    alt={selectedFertilizer[0].name}
                  />
                </div>
                <div
                  className="fertilizer-details"
                  style={{ marginLeft: '20px' }}
                >
                  <Typography
                    variant="subtitle1"
                    className="fertilizer-name"
                    style={{
                      fontSize: '25px',
                      color: 'white',
                  ...
                  >
                    <b>{selectedFertilizer[0].name}</b>
                  </Typography>
                  <Typography variant="body1" className="fertilizer-quantity">
                    <b>Item Code:</b> {selectedFertilizer[0].itemcode}
                  </Typography>
                  <Typography variant="body1" className="fertilizer-price">
                    <b>Price:</b> {selectedFertilizer[0].price}
                  </Typography>
                  <Typography variant="body1" className="fertilizer-quantity">
                    <b>Quantity:</b> {selectedFertilizer[0].quantity}
                  </Typography>
                </div>
              </div>
            )}
        ...
        <div className="section update-listings-section light-green-bg">
          <Typography variant="h4">
            <BarChartIcon
              style={{ fontSize: 36, color: 'black', marginRight: 8 }}
            />
            Quantity Available for each Category
          </Typography>
        ...
        <div className="section update-listings-section light-green-bg">
          ...
          <Table>
            ...
            <TableBody>
              {stableSort(fertilizers, getComparator(sortOrder, sortBy)).map(
                (fertilizer, index) => (
                  <TableRow key={index}>
