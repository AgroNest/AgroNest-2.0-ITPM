...
const ViewTopAreas = () => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:8070/topdealer/update/${updateFormData._id}`,
        updateFormData
      );
  ...
  const generateReport = () => {
    ...
    const pdfData = <MyDocument data={topAreas} />;
  ...
  // Search function
  const filteredAreas = topAreas.filter(
    (area) =>
