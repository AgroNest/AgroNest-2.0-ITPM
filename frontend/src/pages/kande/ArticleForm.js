...
const ArticleForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || title.length > 70 || /\d/.test(title)) {
      setFormError(
        'Title should be less than 71 characters and cannot contain numbers.'
      );
      return;
    }
    try {
      if (selectedArticle) {
        await axios.put(
          `http://localhost:8070/api/articles/${selectedArticle._id}`,
          { title, content }
        );
      } else {
        await axios.post('http://localhost:8070/api/articles', {
          title,
          content,
        });
      }
          ...
          >
            <CardContent>
              <Typography
                variant="h5"
                component="h2"
                style={{ color: '#4CAF50', maxWidth: '100%' }}
              >
                {article.title}
              </Typography>
