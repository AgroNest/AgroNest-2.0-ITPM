  ...
  useEffect(() => {
    const fetchInquiries = async () => {
      try {
        ...
        const farmerPendingInquiries = allInquiries.filter(
          (inquiry) =>
            inquiry.status === 'Pending' && inquiry.category === 'Farmer'
        );
        const farmerResolvedInquiries = allInquiries.filter(
          (inquiry) =>
            inquiry.status === 'Resolved' && inquiry.category === 'Farmer'
        );
        ...
        farmerPendingInquiries.forEach(async (inquiry) => {
          try {
            const replyResponse = await axios.get(
              `http://localhost:8070/dealers/${inquiry._id}/reply`
            );
            // Assuming the replies are stored in a property called 'replies'
            console.log(
              'Replies for inquiry ID',
              inquiry._id,
              ':',
              replyResponse.data
            ); // Log replies
        ...
        <div>
          ...
          {filteredPendingInquiries.map((inquiry) => (
            ...
            >
              ...
              <Grid item xs={2}>
                <Button
                  onClick={() => handleViewInquiry(inquiry, inquiry.replies)}
                  variant="contained"
                  color="success"
                  style={{ marginBottom: '20px' }}
                >
